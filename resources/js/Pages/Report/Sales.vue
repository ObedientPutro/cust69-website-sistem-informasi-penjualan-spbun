...
const summary = computed(() => {
    return props.data.reduce((acc, d) => ({
        ...
        sys_backdate: acc.sys_backdate + (d.sys_backdate || 0),
    ...
    <AdminLayout>
        ...
        <div class="mb-6 flex flex-col xl:flex-row justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400">Rekonsiliasi Liter (Fisik vs Sistem) & Setoran Uang.</p>
            </div>
        ...
        <div class="rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900 overflow-x-auto shadow-sm">
            <table class="w-full text-left text-sm whitespace-nowrap">
                <thead class="bg-gray-100 dark:bg-gray-800 border-b dark:border-gray-700 uppercase text-[10px] text-gray-500 dark:text-gray-400 font-bold tracking-wider text-center">
                <tr>
                    ...
                    <th colspan="3" class="px-4 py-2 border-r dark:border-gray-700 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300">Volume (Liter)</th>
                    <th colspan="5" class="px-4 py-2 border-r dark:border-gray-700 bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-300">Omset Sistem (Rp)</th>
                    <th colspan="2" class="px-4 py-2 bg-orange-50 dark:bg-orange-900/20 text-orange-700 dark:text-orange-300">Realisasi Fisik (Shift)</th>
                </tr>
                <tr>
                    <th class="px-3 py-2 bg-blue-50/50 dark:bg-blue-900/10">Fisik (Mesin)</th>
                    <th class="px-3 py-2 bg-blue-50/50 dark:bg-blue-900/10">Sistem</th>
                    <th class="px-3 py-2 bg-blue-50/50 dark:bg-blue-900/10 border-r dark:border-gray-700">Selisih</th>

                    <th class="px-3 py-2 bg-green-50/50 dark:bg-green-900/10">Cash</th>
                    <th class="px-3 py-2 bg-green-50/50 dark:bg-green-900/10">Transfer</th>
                    <th class="px-3 py-2 bg-green-50/50 dark:bg-green-900/10">Bon</th>
                    <th class="px-3 py-2 bg-yellow-50/50 dark:bg-yellow-900/10 text-yellow-700 dark:text-yellow-400">Backdate</th>
                    <th class="px-3 py-2 bg-green-50/50 dark:bg-green-900/10 border-r dark:border-gray-700">Total</th>

                    <th class="px-3 py-2 bg-orange-50/50 dark:bg-orange-900/10">Uang Laci</th>
                    <th class="px-3 py-2 bg-orange-50/50 dark:bg-orange-900/10">Beda Kas</th>
                </tr>
                </thead>
                ...
                <template v-for="(day, idx) in data" :key="idx">
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50 cursor-pointer transition" @click="toggleRow(day.date)">
                        ...
                        <td class="px-4 py-3 text-center font-bold text-gray-900 dark:text-white">{{ day.sys_liter }}</td>
                        ...
                        <td class="px-4 py-3 text-right font-medium text-yellow-600 dark:text-yellow-400 bg-yellow-50/30 dark:bg-yellow-900/10">
                            {{ day.sys_backdate > 0 ? formatRupiah(day.sys_backdate) : '-' }}
                        </td>

                        <td class="px-4 py-3 text-right font-bold text-gray-900 dark:text-white border-r dark:border-gray-700">{{ formatRupiah(day.sys_total) }}</td>

                        <td class="px-4 py-3 text-right font-bold text-gray-800 dark:text-white">{{ formatRupiah(day.phys_cash) }}</td>
                        <td class="px-4 py-3 text-center">
                                <span :class="day.diff_cash < 0 ? 'text-red-500 dark:text-red-400 font-bold' : 'text-green-500 dark:text-green-400'">
                        ...
                        <td colspan="12" class="p-4 pl-12">
                            <div class="flex flex-col lg:flex-row gap-6">
                                ...
                                <div class="w-full lg:w-1/2">
                                    <h4 class="font-bold text-gray-500 dark:text-gray-400 text-xs mb-2 flex items-center gap-2">
                                    ...
                                    <table class="w-full bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
                                        ...
                                        <template v-for="s in day.shifts" :key="s.id">
                                            <tr class="border-b dark:border-gray-700">
                                                ...
                                                <td class="px-3 py-2 text-right font-bold text-gray-900 dark:text-white">{{ s.total_sales_liter }}</td>
                                    ...
                                    <table class="w-full bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
                                        ...
                                        <tr v-for="t in day.transactions" :key="t.id" class="border-b dark:border-gray-700" :class="{'bg-yellow-50/50 dark:bg-yellow-900/10': t.is_backdate}">
                                            <td class="px-3 py-2 flex items-center gap-2">
                                                ...
                                                <span v-if="t.is_backdate" class="px-1.5 py-0.5 text-[10px] bg-yellow-200 text-yellow-800 dark:bg-yellow-700/50 dark:text-yellow-200 rounded font-bold" title="Transaksi Lampau">BKD</span>
                                            </td>
