...
const inputClasses = computed(() => {
    const base = "h-11 w-full rounded-lg border bg-transparent py-2.5 text-sm shadow-theme-xs placeholder:text-gray-400 focus:outline-hidden focus:ring-3 transition-colors duration-200";

    // Padding kiri (Prefix Slot)
    const paddingLeft = useSlots().prefix ? 'pl-11' : 'pl-4';

    // Padding kanan (Suffix Slot / Error Icon)
    const paddingRight = (useSlots().suffix || props.error || props.success) ? 'pr-11' : 'pr-4';
...
<template>
    <div class="w-full">
        ...
        <div class="relative">
            <div v-if="$slots.prefix" class="absolute left-0 top-1/2 -translate-y-1/2 pl-4 text-gray-500 dark:text-gray-400 pointer-events-none">
                <slot name="prefix" />
            </div>
            ...
            <div class="absolute right-4 top-1/2 -translate-y-1/2 flex items-center pointer-events-none">
                ...
                <div v-if="$slots.suffix" class="pointer-events-auto flex items-center">
                    <slot name="suffix" />
                </div>

                <span v-else-if="error" class="text-error-500">
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M10 18.3333C5.39763 18.3333 1.66667 14.6024 1.66667 10C1.66667 5.39763 5.39763 1.66667 10 1.66667C14.6024 1.66667 18.3333 5.39763 18.3333 10C18.3333 14.6024 14.6024 18.3333 10 18.3333ZM9.16667 12.5V14.1667H10.8333V12.5H9.16667ZM9.16667 5.83333V10.8333H10.8333V5.83333H9.16667Z" fill="currentColor"/>
                    </svg>
                </span>
